...
the Java and the AutoSynch implementation for the variant bounded-buffer
problem (also known as the producer-consumer problem) \cite{dijk65, dijk71}. 
In this problem, two kinds of threads, producers and consumers, try to 
access  the shared resources. Producers put items into the buffer, while 
consumers take items out of the buffer. Both $put$ and $take$
function have a parameter, $num$, indicating the number of items is put or 
taken. There are two requirements for synchronization.
First, every operation on a shared variable, such as $items$, should be done 
...
predicates with condition variables and call signal (signalAll) or await 
function manually.
...
1. For conditional synchronization,  we use $waituntil$ as in line 9. There are 
no {\em signal} or {\em signalAll} calls in the AutoSynch program.
...
\begin{multicols}{2}
    ...
    \begin{Verbatim}[fontsize=\footnotesize,gobble=8,frame=lines,framesep=5mm,
        ...
        AutoSynch class BoundedBuffer { 
          ...
          public void put(int num) { 
            waituntil(count + num <= items.length); 
            for (int i = 0; i < num; ++i) {
              items[putPtr++] = new Object(); 
              if (putPtr == items.length) { 
                putPtr = 0; 
...
Therefore, the $signalAll$ call is used instead of $signal$ calls in
line 23 and 39.
...
the status of the monitor. For example, in Fig.~\ref{fig:bb_exp}, the 
...
predicate is derived from the globalization. For example, in
Fig.~\ref{fig:bb_exp}, the consumer $C$ wants to take $48$ items at this 
moment. Applying the globalization to the general predicate $(count \ge num)$
...
\begin{definition}[Active and inactive thread] 
    ...
    $waituntil$ statement and has not been signaled, then it is an {\em inactive} 
...
AutoSynch uses the following rule for signaling.
...
Proposition \ref{pro:one} shows that the relay
invariance holds in our automatic-signaling mechanism. 
Thus, $signalAll$ calls are avoidable in AutoSynch. 
The remaining problem is how to efficiently find a thread waiting for a condition that is 
true. 
...
used as keys when we do evaluation. 
...
\begin{definition}[Local and general expression]
    ...
    \subseteq L$, then $E$ is a local expression. Otherwise, ($X \not\subseteq L$,)
    $E$ is a general expression.  
\end{definition}
We use $GE$ to denote a
    general expression, and $LE$ to denote a local expression.

\begin{definition}[Equivalence predicate]
    A predicate $P: (GE = LE)$ is an equivalence predicate.
\end{definition}
%\begin{definition}
...
them. Consider the predicate $x = (y - b) / a$ 
where $x, y \in L$ and $a,b \in S$. This predicate is equivalent to 
$ax + b = y$ which is an equivalence predicate. Thus, these two types of predicates can represent a wide range of
...
to get a constant value on the right hand side of the predicate. 
...
%
In AutoSynch, there are three types of tags, $Equivalence$, $Threshold$, and 
...
tag. For example, consider the predicate $x + b*y > a$ where $a$ and $b$ are local variables with values $11$ and $2$.
We first use the globalization to convert it to $(x+2*y > 11)$ and is
represented by the tag $(Threshold,\ x + 2*y,\ 11,\ >)$. The formal definition of
tag is as follows. 
\begin{definition}
...
only when its general expression equals a specific value. 
...
keys. Suppose that these tags are assigned to $m$ predicates. The computation
complexity for maintaining the heap is $O(n\ ln(n))$ since each $insert$ 
operation takes $ln(n)$. However, the performance is generally much better because we only
need to check the predicates of the tags in the root in the most cases; finding
...
the is true, then the thread can continue. Otherwise, the type of
...
whether the derived predicate has been added. If not, we add the 
...
multiple threads evaluate predicate simultaneously may increase the
performance. 
