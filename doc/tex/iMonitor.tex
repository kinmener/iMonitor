...
much better than the generally belief that automatic-signal mechanism is 10-50
times slower than the explicit-signal mechanism \cite{bfc95}. 
Even though AutoSynch is around 2.33 times slower than the explicit-signal 
...
signalAll calls. Besides, the experimental results also shows that AutoSynch is 
scalable; the performance of AutoSynch is stable even if the number of 
threads increases. 
...
The signalAll call is bad since it may decreases the performance because 
...
\begin{definition}{Relay guarantee:}
    some thread waiting for a condition has met is always signaled, i.e., 
    suppose $S_t$ represents the set of threads waiting a condition that has 
    met at time $t$.
    \[
        \forall t,\ if\ S_t \ne \phi,\ \exists T \in S_t,\ T\ is \ signaled.  
    \]
