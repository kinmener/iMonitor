...
waits predicate $P:  (count \ge 24)$ to be true. A producer, $D$, becomes active after 
...
Then $signalAll$ calls are avoidable in AutoSynch since relay invariance holds. 
...
true efficiently, we analyze every waiting condition and assign different tags 
to every predicate according to its semantics. Those tags depict the properties 
of a predicate; therefore, tags
...
Before going through the details, two 
particular types of 
...
$None$. In general, tags are assigned to predicates according to their types.
If a predicate is neither equivalence nor threshold, it acquires the $None$
tags. Every $Equivalence$ or $Threshold$ tag represents a equivalence predicate 
or threshold predicate respectively. For example, the predicate $x + 8 > 11$ is
represented by the tag $(Threshold,\ x + 8,\ 11,\ >)$. The formal definition of
tag is as follows. 
\begin{definition}
   ...
   \end{itemize}
\end{definition}
Note that, we say a tag is true if the predicate representing the tag is 
true; otherwise, we say the tag is false. 
