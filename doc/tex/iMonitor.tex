...
\CopyrightYear{2013}
\copyrightdata{978-1-4503-2014-6/13/06}
%\clubpenalty=10000 
%\widowpenalty = 10000 
...
{\em closure} that enables the predicate evaluation in
...
%investigated closure operation to enable predicate evaluations in any 
...
%\begin{enumerate} 
%    \item We present the closure, relay invariance, and predicate tagging
...
%predicates of waiting threads, an operation called closure is defined as 
%replacing all local variables of
%a predicate by its values at the time. The closure converts a local 
%predicate into a global predicate since all the local variables have been 
%replaced. The closure has an important characteristic that allows a
...
\begin{proof}
 ...
 Our framework never introduces any deadlock by providing {\em relay
 invariance}. 
...
%after applying the {\em closure} at time $t$. 
%
...
Consider an example, suppose there is one predicates, $p = (x < 5)(y = 3) 
\vee\ assert1.isTrue()\ \vee (x > 5)(assert2.isTrue())$, where $x$ and $y$ are
shared variables, $assert1$ and $assert2$ are instances of a user specified 
class having a boolean function $isTrue()$. The predicate $p$ has 3
tags, $(Equivalence,\ y,\ 3,\ null)$ for clause $(x < 5)(y = 3)$, {\em None}
tag for $assert1.isTrue()$, and $(Threshold,\ x, 5, >)$ for clause 
$(x > 5)(assert2.isTrue())$. 
...
tags. If no predicate that is true can be found after checking {\em
Equivalence} 
...
%predicate, the closure has been applied to the local 
%predicate. Every local variable is replaced in the local predicate by its value
%at this point. The closure does not affect on the results of 
%predicate evaluations since other threads cannot change the value of local 
%variables but only global variables. After applying closure to
...
are identical after applying closure. A predicate table, which is implemented by a
...
If there exists a predicate is true, then its corresponding condition variable
...
A predicate must be removed from its tags and the predicate table once it has 
...
Predicates may be reused. Instead of removing those deactivated predicate, 
...
AutoSynch} in this section. We compare the performance of different signaling
...
There are two types of our experiments. The first is a {\em saturation} test 
\cite{bh05}, in which only monitor accessing functions performed. That is, no 
extra work is in the monitor or out of the monitor. The other set of
experiments simulate different workloads of the monitors \cite{bfc95}. For each
monitor operation, there is a fixed time to perform other monitor
...
structures are created in our framework, the memory consumption is 
...
\begin{description}
    ...
    \item [Sleeping barber \cite{dijk65, dijk71}] The problem is analogous 
        ...
        no customer is waiting, the barber goes to sleep. Every customer arrives
        and checks what the barber is doing. If the barber is sleeping, then he 
        wakes up the barber and has a haircut. Otherwise, the customer checks
...
%        sessions at the same time. 
    \item [Dining philosophers \cite{dijk71}] A number of philosophers are 
        ...
        in between every two neighboring philosophers. A philosopher needs to 
        pick two chopsticks at the same time for eating and he does not put 
        down a chopstick until he finishes 
        eating. A philosopher that wants to eat must wait if one of his shared 
        chopsticks is held by another philosopher.
\end{description}
...
number of threads increase. For {\em AutoSynch}, the performance is slower than 
...
the delay time, the amount of time in which the threads perform monitor 
unrelated operations between every two monitor operations, in microseconds. As
can be seen, the performance difference decreases as the duration of delay time
...
performs well when the duration of delay time is $5000$
microseconds. The similar observation can be seen for the readers/writers
...
{\em AutoSynch} uses the {\em closure} operation to enable the complex predicate 
evaluation in every thread. It also provides {\em relay invariance} that some
thread waiting for a condition has met is always signaled to avoid {\em signalAll}
calls. {\em AutoSynch} also uses predicates tag to accelerate the process in 
deciding which thread should be signaled. 
