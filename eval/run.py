...
if __name__ == "__main__":
   ...
   exp_dir = working_dir + "/" + exp_info["result_base_dir"] + "/" + exp_info["name"] + "_" + strftime("%Y%m%d_%H%M%S", localtime())
   os.makedirs(exp_dir)
   ...
   for i in range(0, total_num_comb):
      ...
      if params_idx[group_by_idx] == 0:
         ...
         stat_file = open(exp_dir + "/stat/" + str(params_list[stat_by_idx][params_idx[stat_by_idx]]) + ".dat", "w")
      ...
      min = sys.maxsize
      sum = 0
      raw_file.write(str(params_list[group_by_idx][params_idx[group_by_idx]]) + ", ")
